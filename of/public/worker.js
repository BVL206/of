...
var stopwatch = (function() {
	...
	stopwatch.prototype.stop = function() {
		...
		this.run_time = (this.stop_time - this.start_time).toFixed(3);
		this.running = false;
	}
...
// Vars
var gui,options,ctx,canvasWidth,canvasHeight,good_matches;
var img_u8, img_u8_smooth, screen_corners, num_corners, screen_descriptors, imageData;
var pattern_corners, pattern_descriptors, pattern_preview;
var matches, homo3x3, match_mask, num_matches, debug = false;
...
var img_width = 320*2;
var img_height = 240*2;

img_u8 = new jsfeat.matrix_t(img_width, img_height, jsfeat.U8_t | jsfeat.C1_t);
img_u8_smooth = new jsfeat.matrix_t(img_width, img_height, jsfeat.U8_t | jsfeat.C1_t);
...
onmessage = function(event) {
	...
	if(event.data.input == 0){
		...
		if(event.data.debug) {debug = event.data.debug;}
		imageData = event.data.srcData;
		
		stat.start("grayscale");
		jsfeat.imgproc.grayscale(imageData.data, img_width, img_height, img_u8);
		stat.stop("grayscale");
		
		stat.start("gauss blur");
		jsfeat.imgproc.gaussian_blur(img_u8, img_u8_smooth, event.data.blur|0);
		stat.stop("gauss blur");
		
		jsfeat.yape06.laplacian_threshold = event.data.laplacian_threshold|0;
		jsfeat.yape06.min_eigen_value_threshold = event.data.min_eigen_value_threshold|0;
		
		stat.start("keypoints");
		num_corners = detect_keypoints(img_u8_smooth, screen_corners, event.data.max_keypoints);
		stat.stop("keypoints");
		...
		if(debug) render_corners(screen_corners, num_corners, data_u32, img_width);
		if(pattern_preview) {
			if(debug) render_mono_image(pattern_preview.data, data_u32, pattern_preview.cols, pattern_preview.rows, img_width);
			stat.start("matching");
			num_matches = match_pattern(event.data.match_threshold);
			good_matches = find_transform(matches, num_matches, event.data.num_model_points,event.data.reproj_threshold,event.data.v2,event.data.v3);
			stat.stop("matching");
		}
		if(debug) render_matches(matches,num_matches,data_u32,img_width);
		stat.stop("total");
		
		var data;
		if(debug){
			if(good_matches > 8) data = {dstData: imageData, frame: event.data.frame, fps: stat.log(), homo3x3: homo3x3, pos: event.data.pos, start_time: event.data.start_time, num_corners: num_corners, pattern_xy: pattern_xy, screen_xy: screen_xy, good_matches: good_matches};
			else data = {dstData: imageData, frame: event.data.frame, fps: stat.log(), pos: event.data.pos, start_time: event.data.start_time, num_corners: num_corners, pattern_xy: pattern_xy, screen_xy: screen_xy, good_matches: good_matches};
			postMessage(data, [data.dstData.data.buffer]);
		} else {
			if(good_matches > 8) data = {frame: event.data.frame, fps: stat.log(), homo3x3: homo3x3, pos: event.data.pos, start_time: event.data.start_time, num_corners: num_corners, pattern_xy: pattern_xy, screen_xy: screen_xy, good_matches: good_matches};
			else data = {frame: event.data.frame, fps: stat.log(), pos: event.data.pos, start_time: event.data.start_time, num_corners: num_corners, pattern_xy: pattern_xy, screen_xy: screen_xy, good_matches: good_matches};
			postMessage(data);
		}
	} else if(event.data.input == 1) train(event.data.imageData);
};
...
function render_matches(matches, num_matches, img, step) {
	// Render matches
	function Drawpixel(x,y)
	{
		var pix = (0xff << 24) | (0x00 << 16) | (0x00 << 8) | 0xff;
		var step = img_width;
		var off = (x + y * step);
		img[off] = pix;
	}
	...
	for(var i=0; i < num_matches; ++i)
	{
		...
		line(s_kp.x, s_kp.y, p_kp.x*0.5, p_kp.y*0.5);
	}
}
...
function train (imageData2) {
	var img_u8 = new jsfeat.matrix_t(imageData2.width, imageData2.height, jsfeat.U8_t | jsfeat.C1_t);
	jsfeat.imgproc.grayscale(imageData2.data, imageData2.width, imageData2.height, img_u8);
	var lev=0, i=0;
	var sc = 1.0;
	...
	new_width = img_u8.cols;
	new_height = img_u8.rows;
	...
	jsfeat.imgproc.gaussian_blur(lev0_img, lev_img, 4); // this is more robust
	...
	for(lev = 1; lev < num_train_levels; ++lev) {
		...
		jsfeat.imgproc.gaussian_blur(lev_img, lev_img, 4);
		corners_num = detect_keypoints(lev_img, lev_corners, max_per_level);
		jsfeat.orb.describe(lev_img, lev_corners, corners_num, lev_descr);
...
function match_pattern(match_threshold) {
...
function find_transform(matches, count, num_model_points, reproj_threshold, v2, v3) {
	// motion kernel
	...
	var ransac_param = new jsfeat.ransac_params_t(num_model_points, reproj_threshold, v2, v3);
	pattern_xy = [];
	screen_xy = [];
